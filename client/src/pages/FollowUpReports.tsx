...
import { Mail, Phone, CheckCircle, Clock, AlertCircle, TrendingUp, Eye, MousePointerClick, ArrowRight, Linkedin, Instagram, Facebook, Globe, UserX, CalendarCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
      ...
      {/* Campaign List -- same format as the main Campaigns page: every
          campaign's follow-up activity visible inline, and clicking a
          campaign expands its full report directly underneath THAT
          campaign (same as "View Tracking" on the main Campaigns page),
          instead of jumping to a report block at the bottom of the whole
          page. */}
      <Card className="border-gray-200 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Follow-Up Activity by Campaign</CardTitle>
          <CardDescription>Click a campaign to expand its full report -- every email sent, opened, clicked, and every call made</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-3">
              {campaigns.map((campaign: any) => {
                ...
                return (
                  <div
                    key={campaign.id}
                    className={`border rounded-lg transition-colors ${isSelected ? "border-blue-400 bg-blue-50/30" : "border-border"}`}
                  >
                    <button
                      type="button"
                    ...
                    {/* Report expands directly below THIS campaign */}
                    {isSelected && (
