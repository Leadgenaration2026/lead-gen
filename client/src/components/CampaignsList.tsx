...
interface CampaignsListProps {
  ...
  // Used by the Campaigns page (5) vs. the All Campaigns page (no limit).
  limit?: number;
  title?: string;
  description?: string;
}

export function CampaignsList({ limit, title = "Your Campaigns", description = "Manage your email campaigns and track performance" }: CampaignsListProps) {
  ...
  // One-time cleanup for messages queued under the old "before follow-up
  // #2" trigger, now that queueSocialOutreachForLead only fires on real
  // 3+ opens/clicks -- clears every campaign's stale pending messages at once.
  const handleClearPendingSocial = async () => {
    if (!confirm("Clear all pending LinkedIn/social messages across every campaign? This removes anything queued under the old rule and can't be undone.")) return;
    try {
      const result = await clearPendingSocialMutation.mutateAsync();
      toast.success(`Cleared ${result.cleared} pending message(s)`);
    } catch (error: any) {
      toast.error(error?.message || "Failed to clear pending messages");
    }
  };
  ...
  // campaigns.list is already ordered newest-first, so the first N are the
  // N most recent.
  const visibleCampaigns = typeof limit === "number" ? allCampaigns.slice(0, limit) : allCampaigns;
  const hiddenCount = typeof limit === "number" ? Math.max(0, allCampaigns.length - limit) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          ...
          <Button
            ...
            disabled={clearPendingSocialMutation.isPending}
            onClick={handleClearPendingSocial}
            title="One-time cleanup for messages queued under the old rule"
          >
            <Linkedin className="w-3.5 h-3.5" />
            {clearPendingSocialMutation.isPending ? "Clearing..." : "Clear Pending Social Messages"}
          </Button>
        </CardHeader>
        <CardContent>
          {campaignsQuery.isLoading ? (
            ...
            <div className="space-y-4">
              ...
              {hiddenCount > 0 && (
                ...
                >
                  View All Campaigns ({hiddenCount} more) <ArrowRight className="w-4 h-4" />
                </Button>
              )}
            </div>
          ) : (
      ...
      {/* Call-suggestion and LinkedIn-message popups only ever appear on
          campaign pages (this one, or All Campaigns) -- not globally. */}
      <EngagementPopups />
    </div>
  );
}
