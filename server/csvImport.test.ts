...
describe("CSV Import - Validation Schema Tests", () => {
  it("should validate lead with phoneNumber field", () => {
    const lead = {
      companyName: "Acme Corp",
      ownerName: "John Smith",
      email: "john@acme.com",
      phoneNumber: "+12566779575",
    };
  ...
  it("should reject lead with empty phoneNumber", () => {
    const lead = {
      companyName: "Acme Corp",
      ownerName: "John Smith",
      email: "john@acme.com",
  ...
  it("should handle Seamless.AI CSV with Employee Size column", () => {
    const headers = [
      "Contact Full Name",
    ...
    const parsed = parseCSVRow(row, mapping);
    expect(parsed.companySize).toBe("50-100");
    expect(parsed.phoneNumber).toBe("(256) 677-9575");
  });

  it("should handle Seamless.AI CSV with all location fields", () => {
    const headers = [
      "Contact Full Name",
      "Company Name - Cleaned",
      "Contact Email",
    ...
    expect(parsed.city).toBe("Montgomery");
    expect(parsed.state).toBe("AL");
    expect(parsed.country).toBe("USA");
  });

  it("should handle Seamless.AI CSV with LinkedIn URL", () => {
    const headers = [
      "Contact Full Name",
      "Company Name - Cleaned",
      "Contact Email",
    ...
    expect(parsed.linkedinUrl).toBe("https://www.linkedin.com/in/bob-johnson");
  });

  it("should handle various phone number formats", () => {
    const headers = ["Contact Full Name", "Company Name", "Contact Email", "Contact Phone 1"];
    ...
    phoneFormats.forEach(phone => {
      const row = ["John Doe", "Test Corp", "john@test.com", phone];
      const mapping = parseCSVHeader(headers);
      const parsed = parseCSVRow(row, mapping);

      expect(parsed.phoneNumber).toBe(phone);
      const result = validateLead(parsed);
      expect(result.valid).toBe(true, `Phone format "${phone}" should be valid`);
    });
  });
});
