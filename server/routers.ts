...
Respond in this exact JSON format:
{
  ...
  leadSets: router({
    ...
    // Get the IDs of the first N (oldest first), or every, untagged lead
    // from an imported list -- queried and limited directly in the database
    ...
    // many leads are currently paginated into view on the Leads page. Used
    // to populate the Leads page's checkbox selection (a "Select First N"
    // helper); actual tag assignment goes through the existing
    // leadSets.assignLeads mutation on whatever ends up selected, same as
    // a manual checkbox selection would.
    getFirstNLeadIdsFromList: protectedProcedure
      .input(z.object({
